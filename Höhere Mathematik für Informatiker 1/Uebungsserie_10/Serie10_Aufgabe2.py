...
DL_invert = np.linalg.inv(D+L)

print("(D+L)^(-1): " + str(DL_invert))
...
b)
'''
def Gauss_Seidel_Verfahren_Matrix(x, k):
    counter = 0
    print("x^(" + str(counter) + "): " + str(x))
    while counter < k:
        counter += 1
        x = - DL_invert@R@x + DL_invert@b
        print("x^(" + str(counter) + "): " + str(x))
    return x
k_3 = 3
k_2 = 2
x_2 = Gauss_Seidel_Verfahren_Matrix(x, k_2)
x_3 = Gauss_Seidel_Verfahren_Matrix(x, k_3)
...
k_1 = 1
x_1 = Gauss_Seidel_Verfahren_Matrix(x, k_1)
a_priori = 1e-4
...
def A_Prorio_Abschätzung_Unendlich_Anzahl_Iterationen(B, x1, x0, a_priori):
    ...
    das gleiche wenn man durch negative zahl teilt
    '''
